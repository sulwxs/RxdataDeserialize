...
namespace Decoder
{
    ...
    using System.Windows.Forms;
        ...
        public class DecoderXml
        {
            ...
            public  bool tryReadVer(Stream stream)
            {

                if (stream.ReadByte() == 0x04 && stream.ReadByte() == 0x08)
                    return true;
                else
                {
        ...
        {
            byte hexchar = (byte)match.Value.ToCharArray()[0];
            return string.Format("\\x{0:x2}", hexchar);
        ...
        public String BytetoHexString(byte[] bytes)
            ...
            byte t = (byte)stream.ReadByte();
                switch (t)
                {
                    ...
                    case 0x49:
                        ...
                        break;
                    case 0x75:
                        re += decodeUser_dump(stream);// _dump
                        break;
                    case 0x55:
                        re += decodeUserMarshal_dump(stream);//marshal_dump
                        break;
            ...
            string re = ReplaceHexSymbols(System.Text.Encoding.UTF8.GetString(buff));
            ...
            public String decodeFloat(Stream stream)
            {
                ...
                byte[] buff = new byte[len];
                stream.Read(buff, 0, len);
             return addTag("Float",ReplaceHexSymbols( System.Text.Encoding.UTF8.GetString(buff)));
             }
