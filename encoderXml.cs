    ...
    public class EncoderXml
    {
            ...
            switch (node.Name)
            {
               ...
               //     encodeTrueClass(list);//
                ...
                case "I":
                    ...
                    break;
                case "Userdump":
                    encodeUser_dump(list, node);// _dump
                    break;
                case "UserMarshal_dump":
                    encodeUserMarshal_dump(list);// marshal_dump
                    break;
        ...
        public void encodeHash(List<byte> list, XmlNode node)
        {
            if (node.SelectNodes("defaultValue").Count > 0)
            {
                ...
                XmlNodeList nlist = node.SelectNodes("Item");
                int len = nlist.Count;
                encodeLen(list, len);
                foreach (XmlNode xmlNode in nlist)
                {
        ...
        public void encodeFixnum(List<byte> list, XmlNode node)
        {
            ...
            if (cnt < 1)
                ...
                numl.AddRange(bytes.Take<byte>(cnt+1));
            if (num < -123)
            {
        ...
        public void encodeBool(List<byte> list,XmlNode node)
        {
            if (node.InnerText.ToLower().Trim() == "false")
                list.Add((byte)'F');
            else if (node.InnerText.ToLower().Trim() == "true")
                list.Add((byte)'T');
        }
        ...
        public static string ReplaceHex(Match match)
        {
            char hexchar = (char)byte.Parse( match.Value.Replace("\\x",""),System.Globalization.NumberStyles.HexNumber);
            return  hexchar.ToString();
           
        }
        public static string ReplaceHexSymbols(string txt)
        {
            MatchEvaluator evaluator = new MatchEvaluator(ReplaceHex);
            string r = @"\\x[0-9|a-f|A-F]{1,2}";
            return Regex.Replace(txt,r,evaluator);
        }
        public void encodeString(List<byte> list, XmlNode node)
        {

            list.Add(0x22);
            byte[] bytes = Encoding.UTF8.GetBytes(ReplaceHexSymbols(node.InnerText));
        ...
        public void encodeFloat(List<byte> list, XmlNode node)
        {
            ...
            byte[] bytes = Encoding.ASCII.GetBytes(ReplaceHexSymbols(node.InnerText));
                int len = bytes.Length;
            encodeLen(list, len);
            list.AddRange(bytes);
